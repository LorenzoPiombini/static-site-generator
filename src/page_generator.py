...
def generate_page(from_path,template_path,dest_path,basepath):
    ...
    p = p.replace('href="/',f'href="{basepath}')
    p = p.replace('src="/',f'src="{basepath}')
    with open(dest_path,"w") as index:
        index.write(p)
        index.close()

def gen_pages(from_path:str,temp:str,dest:str,basepath:str):
    for d in os.listdir(from_path):
        ...
        if os.path.isfile(path):
            ...
            generate_page(path,temp,dest_final_path,basepath)
        else:
            dest_dir = os.path.join(dest,d)
            if os.path.exists(dest_dir) == False:
                os.mkdir(dest_dir)
            gen_pages(path,temp,dest_dir,basepath)

def generate_pages_recursive(working:str,dir_path_content:str, template_path:str, dest_dir_path:str,basepath):
    try:
        if is_path_valid(working,dir_path_content) and is_path_valid(working,dir_path_content) and is_path_valid(working,template_path) and is_path_valid(working,dest_dir_path):
            gen_pages(dir_path_content,template_path,dest_dir_path,basepath) 
